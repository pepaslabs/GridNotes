...
class GridKeyboardViewController: UIViewController, InterfaceDelegating {
    ...
    private var _depressedNotes: Set<AbsoluteNote> {
        var depressed = Set<AbsoluteNote>()
        for row in _keyRows {
            depressed = depressed.union(row.depressedNotes)
        }
        return depressed
    }
    
    private var _implicitlyDepressedNotes: Set<AbsoluteNote> {
        guard state.chordMode else { return Set<AbsoluteNote>() }
        var implicit = Set<AbsoluteNote>()
        for note in _depressedNotes {
